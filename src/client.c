...
PlayerClient::PlayerClient(const std::shared_ptr<LevelStage>& stage,
                       ...
                       const std::shared_ptr<ClientStats>& stats,
                       int flip)
    : Client(stage, texture_key, position, velocity, stats, flip) {}

Player::Player(const std::shared_ptr<LevelStage>& stage)
    : client(std::make_shared<PlayerClient>(stage,
                                        "client1",
                                        SDL2pp::Point{30, 30},
                                        SDL2pp::Point{0, 0})) {}
...
void PlayerClient::HandleInput(const std::shared_ptr<InputManager>& input) {
...
Client::Client(const std::shared_ptr<LevelStage>& stage,
           const std::string& texture_key,
           SDL2pp::Point position,
           SDL2pp::Point velocity,
           const std::shared_ptr<ClientStats>& stats,
...
void Client::Update() {
  if (GetHit()) {
    renderer->Copy(
        *resource_manager->GetTexture("Client1_hit"),
